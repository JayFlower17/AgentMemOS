...
const translations = {
  en: {
    ...
    "legend.memory.sourceKinds": "<strong>Write path tells how a memory entered the system.</strong>",
    "legend.memory.manual": "<strong>manual</strong>: created directly through the memory API, usually with sparse source signals.",
    "legend.memory.extracted": "<strong>extracted</strong>: derived by the worker from an agent event, with source event and classification signals.",
    "legend.memory.deduplicated": "<strong>deduplicated</strong>: a repeat write matched an existing active memory, so the system reused it and recorded the merge decision.",
    ...
    "legend.memory.relations": "<strong>Governance relations explain how memories affect each other.</strong>",
    "legend.memory.relationSupersedes": "<strong>supersedes</strong>: the source memory is newer or more authoritative, and the target memory is automatically marked superseded.",
    "legend.memory.relationConflicts": "<strong>conflicts_with</strong>: two memories disagree and should be reviewed before agents rely on both.",
    "legend.memory.relationDuplicates": "<strong>duplicates</strong>: two memories carry the same meaning and should be merged or kept as one canonical record.",
    "legend.memory.relationDirection": "<strong>Source/target direction</strong>: source is the memory making the claim; target is the memory being replaced, challenged, or matched.",
  ...
  zh: {
    ...
    "legend.memory.sourceKinds": "<strong>写入路径说明这条 memory 是怎么进入系统的。</strong>",
    "legend.memory.manual": "<strong>manual</strong>：通过 memory API 直接创建，通常来源信号较少。",
    "legend.memory.extracted": "<strong>extracted</strong>：由 worker 从 agent event 中抽取，带有来源事件和分类信号。",
    "legend.memory.deduplicated": "<strong>deduplicated</strong>：重复写入命中了已有 active memory，系统复用旧记录，并追加一条合并决策。",
    ...
    "legend.memory.relations": "<strong>治理关系说明 memory 之间如何互相影响。</strong>",
    "legend.memory.relationSupersedes": "<strong>supersedes</strong>：source memory 更新或更权威，target memory 会自动标记为 superseded。",
    "legend.memory.relationConflicts": "<strong>conflicts_with</strong>：两条 memory 结论不一致，需要复核后再让 agent 同时依赖。",
    "legend.memory.relationDuplicates": "<strong>duplicates</strong>：两条 memory 表达相同含义，应合并或保留一条 canonical 记录。",
    "legend.memory.relationDirection": "<strong>source/target 方向</strong>：source 是提出关系的 memory，target 是被替代、被挑战或被匹配的 memory。",
